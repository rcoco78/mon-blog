...
export async function fetchPageMeta(pagePath) {
  ...
  try {
    ...
    if (source.source === 'cas-usage') {
      const slug = source.identifier
      // Essayer d'abord case-studies/[slug].json (~100 KB vs 4.5 MB)
      try {
        const slugBlob = await head(`case-studies/${slug}.json`)
        if (slugBlob) {
          const res = await fetch(slugBlob.url, { cache: 'no-store' })
          if (res.ok) {
            const cs = await res.json()
            return { title: cs.metaTitle || cs.title || '', metaDescription: cs.metaDescription || '' }
          }
        }
      } catch { /* fichier absent, fallback full */ }
...
export async function applyOptimizationToBlob(pageSource, suggestion) {
  ...
  try {
    ...
    if (pageSource.source === 'cas-usage') {
      ...
      caseStudies[idx] = updated
      const lastUpdated = data.lastUpdated || new Date().toISOString()
      await put(
        'case-studies.json',
        JSON.stringify({ caseStudies, lastUpdated }, null, 2),
        { access: 'public', allowOverwrite: true },
      )
      // Mettre à jour aussi case-studies/[slug].json pour les pages détail (format split)
      await put(`case-studies/${slug}.json`, JSON.stringify(updated, null, 2), {
        access: 'public',
        allowOverwrite: true,
      })
      return { applied: true }
