 ...
 * Utilise l'API Vercel Blob directement au lieu de fetch() pour éviter les problèmes de cache
 */
async function getClickEvents() {
  try {
    ...
    // Utiliser l'API Vercel Blob directement au lieu de fetch() pour éviter le cache
    const blob = await get(existingBlob.pathname)
