...
export async function addClickEvent(projectId) {
  try {
    ...
    // Utiliser list() pour obtenir l'URL directement et éviter le cache
    let currentClicks = {}
    try {
      ...
      if (existingBlob) {
        ...
        const response = await fetch(`${existingBlob.url}?t=${cacheBuster}&_=${Date.now()}`, {
          method: 'GET',
          cache: 'no-store',
          headers: {
        ...
        if (response.ok) {
          currentClicks = await response.json() || {}
        }
      }
    } catch (error) {
      console.warn('Error fetching current clicks, starting from 0:', error)
      currentClicks = {}
    }
