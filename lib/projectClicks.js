...
export async function incrementProjectClick(projectId) {
  ...
  while (retries < MAX_RETRIES) {
    try {
      ...
      // Si on a une URL existante, vérifier qu'elle n'a pas changé entre temps
      if (url) {
        try {
          // Vérifier que le blob n'a pas été modifié entre temps
          const currentBlob = await head(BLOB_FILENAME)
          if (currentBlob && currentBlob.url !== url) {
            // Le fichier a été modifié, réessayer
            retries++
            await new Promise(resolve => setTimeout(resolve, RETRY_DELAY * retries))
            continue
          }
        } catch (headError) {
          // Si head échoue, continuer quand même (fichier peut ne pas exister)
        }
      }
