 ...
 */
export function generateSEOTitle(pageTitle, includeSiteName = true, maxLength = 60) {
  if (!pageTitle) return siteConfig.title
  
  const baseTitle = includeSiteName ? `${pageTitle} | ${siteConfig.name}` : pageTitle

  if (baseTitle.length <= maxLength) return baseTitle

  // Tronquer proprement au dernier espace avant maxLength
...
export function generatePageSEO({
  ...
  // Pas de suffixe ici : SEOHead ajoute déjà « | Corentin Robert »
  const seoTitle = generateSEOTitle(title, false)
