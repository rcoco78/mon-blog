...
// Obtenir les bases de données proposées comme add-ons (addonSlugs)
export async function getAddonDatabases(slug) {
  const current = await getDatabaseBySlug(slug)
  if (!current?.addonSlugs?.length) return []

  const databases = await getAllDatabases()
  return databases.filter(db => current.addonSlugs.includes(db.slug))
}
...
export async function getDatabasesAsTools() {
  ...
  // Importer categoryToSlug pour construire les liens avec catégorie
  const { categoryToSlug } = await import('./marketplace-helpers')
  
  return databases.map(db => {
    // S'assurer que la catégorie existe et générer un slug valide
    const category = db.category || 'autres'
    const categorySlug = categoryToSlug(category) || 'autres'
    
    const shortDescription = db.shortDescription || ''
    const headers = Array.isArray(db.headers) ? db.headers : []
    const benefitFromHeaders = headers.length > 0
      ? `${headers.slice(0, 4).join(', ')}${headers.length > 4 ? '…' : ''} — prêts pour la prospection.`
      : ''

    return {
      slug: db.slug,
      name: db.name,
      description: db.description || shortDescription || '',
      shortDescription: shortDescription || benefitFromHeaders,
      benefit: shortDescription
        ? shortDescription.split(/(?<=[.!?])\s+/)[0]
        : benefitFromHeaders,
      category: category,
      type: 'database',
      iconSvg: 'search',
      ...
      isPaid: !!db.isPaid,
      price: db.isPaid && db.price != null ? db.price : null,
      annualPrice: db.annualPrice != null ? db.annualPrice : null,
      rowCount: db.rowCount || 0,
      headers: headers || [],
      isNew: true,
      date: db.lastEnriched
        ? db.lastEnriched.split('T')[0]
