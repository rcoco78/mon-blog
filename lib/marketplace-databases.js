...
export async function getAllDatabases() {
  ...
  try {
    ...
    const timeoutPromise = new Promise((_, reject) => 
      setTimeout(() => reject(new Error('Timeout Blob Storage')), 2000)
    )
...
// Exporter pour compatibilité avec tools.js (version synchrone pour getStaticProps)
export function getDatabasesAsTools() {
  const databases = getAllDatabasesSync()
