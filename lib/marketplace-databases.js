...
export async function getAllDatabases() {
  console.log('🔍 getAllDatabases() appelé')
  console.log('  - typeof window:', typeof window)
  console.log('  - fs disponible:', !!fs)
  console.log('  - DATABASES_FILE:', DATABASES_FILE)
  console.log('  - BLOB_READ_WRITE_TOKEN:', !!process.env.BLOB_READ_WRITE_TOKEN)
  console.log('  - VERCEL:', !!process.env.VERCEL)
  
  // Fonction uniquement disponible côté serveur
  if (typeof window !== 'undefined') {
    console.warn('⚠️  getAllDatabases() appelé côté client, retourne []')
    return []
  }
          ...
          if (response.ok) {
            ...
            if (data.databases && Array.isArray(data.databases)) {
              if (data.databases.length > 0) {
                ...
                console.warn('⚠️  Blob Storage contient un tableau vide (0 bases), fallback vers fichier local')
            ...
            // Format inconnu
            console.warn('⚠️  Format Blob Storage inconnu, fallback vers fichier local')
  ...
  if (!process.env.VERCEL && fs && DATABASES_FILE) {
    ...
    console.warn('⚠️  En production Vercel: Blob Storage vide et fichier local non disponible')
  }
  
  console.warn('⚠️  Aucune base de données trouvée')
  return []
}
...
export async function getDatabasesAsTools() {
  console.log('🔍 getDatabasesAsTools() appelé')
  const databases = await getAllDatabases()
  console.log(`📊 ${databases.length} base(s) de données récupérée(s) dans getDatabasesAsTools`)
  
  if (!databases || databases.length === 0) {
    console.warn('⚠️  Aucune base de données trouvée dans getDatabasesAsTools')
    return []
  }
  
  const tools = databases.map(db => {
    if (!db.slug) {
      console.warn(`⚠️  Base de données sans slug: ${db.name || 'sans nom'}`)
    }
    return {
  ...
  console.log(`✅ ${tools.length} outil(s) généré(s) depuis les bases de données`)
  return tools
}
