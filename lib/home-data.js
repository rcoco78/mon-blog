...
async function fetchMetrics() {
  try {
    ...
    if (data?.metrics) {
      return await enrichMetricsWithApifyLive(data.metrics)
    }
  } catch (e) {
    captureDataError(e, { source: 'blob', tags: { area: 'metrics' } })
    console.warn('home-data: metrics fetch error', e.message)
  }
  return enrichMetricsWithApifyLive(siteConfig.metrics)
}
...
async function fetchKeyResults() {
  try {
    ...
    if (data?.keyResults && Array.isArray(data.keyResults)) {
      return await enrichKeyResultsWithApifyLive(data.keyResults)
    }
    const fromNotion = await withTimeout(getKeyResults(), FETCH_TIMEOUT_MS, [])
    return await enrichKeyResultsWithApifyLive(fromNotion)
  } catch (e) {
...
function selectTopBusinessPosts(posts, viewsMap, limit = 3) {
  ...
  const business = withViews.filter(isBusinessPost).sort((a, b) => b.views - a.views)
  if (business.length >= limit) return business.slice(0, limit)

  const fallback = withViews
    .filter((p) => !isLifestylePost(p) && !business.some((b) => b.slug === p.slug))
    .sort((a, b) => b.views - a.views)

  return [...business, ...fallback].slice(0, limit)
}
