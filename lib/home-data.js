...
export async function fetchHomeData(posts = []) {
  ...
  const latestPosts = datedPosts.slice(0, siteConfig.homepage.topPostsCount || 3).map((p) =>
    lightPost({ ...p, views: viewsMap[p.slug] || p.views || 0 })
  )

  const latestPost = datedPosts.slice(0, 1).map(lightPost)[0] || null

  return {
    metrics,
    postsCount: datedPosts.length,
