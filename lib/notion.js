...
export async function getPostBlocks(pageId) {
  try {
    ...
    // Les tableaux Notion stockent leurs lignes en enfants : les rattacher
...
export async function getMetrics() {
  try {
    ...
    // Ajouter la métrique totale des projets — Malt + Fiverr (profils publics)
    const maltPublic = siteConfig.socialProof?.malt?.projects || 183
    const fiverrPublic = siteConfig.socialProof?.fiverr?.projects || 0
    if (totalProjects > 0 || maltValue > 0 || fiverrValue > 0 || maltPublic > 0) {
      const malt = maltPublic
      const fiverr = fiverrPublic > 0 ? fiverrPublic : (fiverrValue > 0 ? fiverrValue : 0)
    ...
    // Ordre attendu des métriques sur le site (fallback = Malt + Fiverr publics)
    const maltProjects = siteConfig.socialProof?.malt?.projects || 183
    const fiverrProjects = siteConfig.socialProof?.fiverr?.projects || 0
    const expectedOrder = [
      {
        label: 'projets réalisés',
        value: String(maltProjects + fiverrProjects),
        source: fiverrProjects > 0 ? 'Malt & Fiverr' : 'sur Malt',
        href: siteConfig.social.malt,
        breakdown: fiverrProjects > 0 ? { malt: maltProjects, fiverr: fiverrProjects } : null,
      },
