...
export default function FloatingCalendlyButton() {
  ...
  const openCalendly = () => {
    ...
    if (!calendlyLoaded) {
      ...
      // Si déjà chargé, ouvrir directement
      if (window.Calendly) {
        window.Calendly.initPopupWidget({
          url: 'https://calendly.com/corentinrobert/20min'
        })
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      <button
        ...
        className="p-4 bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 rounded-full shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-110 focus:outline-none focus:ring-2 focus:ring-neutral-400 dark:focus:ring-neutral-600 relative"
        aria-label="Réserver un appel gratuit sur Calendly"
      >
        {/* Effet de déflagration */}
