    ...
    <div ref={containerRef} className="relative">
      ...
      {open && (
        <ul
          role="listbox"
          aria-labelledby={`${id}-label`}
          className="absolute z-20 mt-1 left-0 min-w-[160px] py-1 rounded-lg border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 shadow-lg overflow-hidden"
        >
          {options.map((opt) => (
            <li
              key={opt.value}
